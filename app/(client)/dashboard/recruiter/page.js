...
export default function RecruiterDashboard() {
  ...
  const [showNotificationsPanel, setShowNotificationsPanel] = useState(false); // New state for notifications panel
  const [isLoading, setIsLoading] = useState(true);

  const user = session?.user;
  const isAdmin = user?.recruiterProfile?.recruiterType === "ADMIN";
  let playInterval = null;
  // Define playNotificationSound here
  const playNotificationSound = () => {
    const audio = new Audio("/notificationSound2.mp3"); // Ensure this path is correct
    audio.play().catch((error) => {
      console.error("Failed to play notification sound:", error);
      // This catch block often indicates a browser autoplay policy issue.
      // Advise the user to interact with the page if sound is crucial.
    });
  };
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      fetchDashboardData();
    }, 3 * 60 * 1000); // Changed to 3 minutes in milliseconds

    // Cleanup interval on component unmount
    return () => clearInterval(interval);
  }, [session, status, router]); // Dependencies array
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (notificationsResponse.ok) {
        ...
        // Filter out old notifications if you want a 'clear' effect every fetch
        // For example, keep only notifications from the last 3 minutes or a fixed number
        const now = Date.now();
        ...
        // OR simply set all fetched notifications:
        ...
        // Changed logic: Play sound if there are *any* unread notifications,
        // or if the unread count has increased.
        if (unreadCount > 0 && unreadCount > stats.unreadNotifications) {
          playInterval = setInterval(() => {
            playNotificationSound();
          }, 20000);
        } else if (unreadCount > 0 && stats.unreadNotifications === 0) {
          // This handles the case where the user marked all as read,
          // and then a new notification comes in, triggering the sound.
          playInterval = setInterval(() => {
            playNotificationSound();
          }, 20000);
        } else {
          console.log("No new unread notifications or sound already playing");
      ...
      setStats((prevStats) => ({
        ...
        newApplications: 12, // This seems hardcoded; consider fetching real data
      }));
    } catch (error) {
  ...
  const markNotificationAsRead = async (notificationId) => {
    try {
      ...
      if (response.ok) {
        ...
        console.log("Notification marked as read:", playInterval);
        clearInterval(playInterval); // Clear the sound interval when a notification is read
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {notifications.map((notification) => {
          // toast.success(notification); // This line is commented out, but would typically display a toast message.
        })}

        {/* Tab Navigation - Removed notifications tab */}
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              ...
                              <div className="flex items-center gap-2 mt-2">
                                <span
                                  className={`px-3 py-1 rounded-full text-xs font-medium ${getRecruiterTypeColor(
      ...
      {/* Fixed Floating Notifications Button */}
      <motion.div
        initial={{ scale: 0 }}
        animate={{ scale: 1 }}
